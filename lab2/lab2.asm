...
read_key:
	...
	cmp ah, 0x0e				; BACKSPACE pressed
	je input_bksp
	
	cmp ah, 0x1c				; ENTER pressed
	je input_enter
 
	jmp echo_char				; Always read for keyboard inputs
...
; Int 10h 0aH: Write Character to Cursor Location
	mov ah, 0aH					; Overwrite existing character
...
; Int 10h 1301H: Display String & Update Cursor
print_echo:
	...
	mov bl, 0x07				; Attribute: Black on White
...
; Int 10h 0eH: Write character as TTY
	mov ah, 0eH					; Echo any valid characters to screen
	int 10h
	jmp read_key

buffer times 256 db 0		; Empty 256 char buffer for our code
